...
6. When initializing the thesis workspace, use Chinese directory names for all user-facing main folders by default.
...
#### Required Chinese Main Directory Names

When creating a fresh thesis workspace, the main project folders must default to these Chinese names:
...
Inside `04-写作`, the default subfolder should be:

- `分节草稿`

Do not replace these user-facing thesis folders with English names such as:
...
English names are acceptable only for internal skill packaging, code, references, or tool folders. They are not acceptable as the default thesis project main directories created for the user.
